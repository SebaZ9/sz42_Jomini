...
namespace JominiEngine
{
    ...
    public abstract class Character : IEquatable<Character>
    {
        ...
        /// <summary>
        ///     Find the travel cost from the character position to the destination by using the shortest path.
        /// </summary>
        /// <param name="fiefTo">Destination</param>
        /// <returns>Total cost for travelling to this fief with the shortest path</returns>
        public double GetPathCost(Fief fiefTo) {
            Queue<Fief> path = Globals_Game.gameMap.GetShortestPath(this.location, fiefTo);
            double pathCost = 0;
            Fief currentFief = path.Dequeue();
            while (path.Count > 0) {
                Fief nextFief = path.Dequeue();
                pathCost += currentFief.getTravelCost(nextFief, this.armyID);
        ...
        public string[] FindAvailableTravelDirections() {
            ...
            foreach (string direction in correctDirections)
                if (Globals_Game.gameMap.GetFief(this.location, direction) != null)
       ...
       public void MoveTo(string fiefID, out ProtoMessage error)
       {
        ...
        public bool SpyOn(Fief fief, out ProtoMessage result) {
            ...
            if (isSuccessful && wasDetected) {
                ...
                result = fiefDetails;
                result.ResponseType = DisplayMessages.SpySuccessDetected;
                ...
                fiefDetails.includeSpy(fief);
                result = fiefDetails;
                result.ResponseType = DisplayMessages.SpySuccess;
                ...
                result.ResponseType = DisplayMessages.SpyFailDead;
